...
void main() {
  ...
  group('AdapterTvReachabilityService', () {
    ...
    test('prefers exact (brand + variant) match over brand-only fallback',
        () async {
...
class _ReachableAdapter implements TvBrandAdapter {
  ...
  @override
  Stream<bool> watchRemoteTextInputReady(TvDevice device) => const Stream.empty();

  @override
  Future<TvDeviceInfo?> queryDeviceInfo({required TvDevice device}) async => null;
