...
void main() {
  group('DeviceScopedSecretGateway', () {
    test('writes to device-scoped store when stableId is registered for host',
        () async {
    ...
    test('reads device-scoped value first when stableId is registered',
        () async {
      final host = LegacyInMemoryHostScopedSecretPersistence();
      final device = InMemoryDeviceScopedSecretPersistence();
      final registry = DeviceIdentityRegistry()..register('10.0.0.5', 'roku-1');
    ...
    test('falls back to host-scoped value when device-scoped is empty', () async {
      final host = LegacyInMemoryHostScopedSecretPersistence();
      final device = InMemoryDeviceScopedSecretPersistence();
      final registry = DeviceIdentityRegistry()..register('10.0.0.5', 'roku-1');
      final gateway = DeviceScopedSecretGateway(
    ...
    test('falls back to host-scoped when no stableId is registered (legacy)',
        () async {
      final host = LegacyInMemoryHostScopedSecretPersistence();
      final device = InMemoryDeviceScopedSecretPersistence();
    ...
    test('survives IP change: secret written under stableId reads at new host',
        () async {
      final host = LegacyInMemoryHostScopedSecretPersistence();
      final device = InMemoryDeviceScopedSecretPersistence();
      final registry = DeviceIdentityRegistry()..register('10.0.0.5', 'roku-1');
