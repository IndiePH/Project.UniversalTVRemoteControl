...
- **TVREMOTE-63** — Free-tier ads: Unity LevelPlay banner (`unity_levelplay_mediation` 9.2.0, app key `27c78d0ad`, unit `20azo5e9eecpv182`) replaced the AdMob/UMP scaffold (**Done** in repo on `ads-rework`; interstitial not wired). Firebase Remote Config `test_ads_enabled` removed from the app and cleared on project `oneremote-497701`. Full SKAdNetwork list for extra mediated networks still **TVREMOTE-26**.
...
- [x] Android `play-services-auth` override (2026-08-27): pin **21.6.0** in `android/app/build.gradle.kts` — avoids `SignInHubActivity` NPE on transitive **20.7.0** from Firebase Auth
