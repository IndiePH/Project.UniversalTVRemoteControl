...
**Proposed fix:** Introduce `get_it` + `injectable`.
...
| `IDIConfig<Container, AppSettings>` | `@module abstract class XxxModule` |
| `DIBootstrap.Initialize()` | `configureDependencies()` (generated by build_runner) |
| `container.Register<IFoo, Foo>()` | `@singleton` / `@injectable` annotation on `Foo` |
| Assembly scanning via reflection | `build_runner` scans annotations at build time |
| `container.Verify()` | Compile-time type checking by the code generator |
| `AppEnvironment.Production` | `@prod` / `@dev` annotation on a registration |

C# discovers `IDIConfig` implementations at runtime via reflection. Dart disables reflection
in release builds, so `injectable` does the same thing at build time — it scans all `@module`
classes and generates a `*.config.dart` file with the registration calls. Running
`flutter pub run build_runner build` once produces that file; after that `configureDependencies()`
is a plain function call with no magic.

**What the code looks like:**

```dart
// lib/app/modules/lg_module.dart
@module
abstract class LgModule {
  @prod @singleton
  LgTransportClient realClient() => LgWebSocketTransportClient(
        hostResolver: _resolveHost,
        keyStore: LgPairingKeyStore(),
      );

  @dev @singleton
  LgTransportClient fakeClient() => FakeLgTransportClient();

  @singleton
  TvBrandAdapter lgAdapter(LgTransportClient client) =>
      LgAdapter(transportClient: client);
}

// lib/app/injection.dart
@InjectableInit()
void configureDependencies({String env = Environment.prod}) =>
...
class OneRemoteApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: RemoteHomePage(
        commandService: getIt<RemoteCommandService>(),
        deviceRepository: getIt<DeviceRepository>(),
        discoveryService: getIt<DeviceDiscoveryService>(),
...
**Packages:** `get_it: ^9.2.1`, `injectable: ^3.0.0` (dev: `injectable_generator: ^3.0.0`)
**License:** MIT. **Publishers:** verified (`flutter-it.dev`, `codeness.ly`).
**Maintenance:** active — injectable 3.0.0 released April 2026.
