...
final class AppMonetizationDiConfig implements IDiConfig {
  ...
  void configure(GetIt sl, AppEnvironment env) {
    ...
    final receiptValidationService = _needsReceiptValidation
        ? ProReceiptValidationService()
        : null;
    final repository = _buildRepository(receiptValidationService);
  ...
  /// Personal builds skip the store entirely, so there is nothing to validate.
  static bool get _needsReceiptValidation =>
      !AppBuildConfig.personalBuildUnlock &&
      _supportsReceiptValidation &&
      Firebase.apps.isNotEmpty;

  static ProEntitlementRepository _buildRepository(
    ProReceiptValidationService? receiptValidationService,
  ) {
    if (AppBuildConfig.personalBuildUnlock) {
      return FakeProEntitlementRepository(
        initialStatus: ProEntitlementStatus.entitled,
      );
    }
    if (!_supportsInAppPurchaseStore) {
      return FakeProEntitlementRepository(isAvailable: false);
    }
    return StoreProEntitlementRepository(
